...
public class TruckCompositionController extends Controller {
	...
	static Form<TruckComposition> truckCmpForm = Form
			.form(TruckComposition.class);
	...
	public Result createTruckCompositionView() {
		...
		return ok(createTruckCompView
				.render(availableTrucks, availableTrailers));
	}
	...
	public Result attachTrailer(long tcId) {
		...
		Form<TruckComposition> attachTrailerForm = Form.form(
				TruckComposition.class).bindFromRequest();
		String trailerName;
		try {
			...
			for (int i = 0; i < vids.length; i++) {
				vi = vids[i];
				System.out
						.println("ISPISUJEM NIZ ID STRINGOVA U ATTACH_TRAILER METODI:"
								+ vi);
			...
			Logger.error("Creating truck composition error: " + e.getMessage(),
					e);
			System.out
					.println("ERROR ATTACHING TRAILER////////////////////////////"
							+ e.getMessage());
	...
	public Result editTruckComposition(long id) {
		...
		Form<TruckComposition> form = Form.form(TruckComposition.class)
				.bindFromRequest();
		TruckComposition tc = TruckComposition.findById(id);
		try {
			...
			/*
			 * v.vid = vehicleForm.bindFromRequest().data().get("vid"); String
			 * ownerName =
			 * vehicleForm.bindFromRequest().data().get("ownerName");
			 * 
			 * String ownerEmail =
			 * vehicleForm.bindFromRequest().data().get("ownerEmail");
			 * 
			 * 
			 * String typeName =
			 * vehicleForm.bindFromRequest().data().get("typeName");
			 * 
			 * String description =
			 * vehicleForm.bindFromRequest().data().get("typeDescription");
			 * 
			 * String fleetName =
			 * vehicleForm.bindFromRequest().data().get("fleetName");
			 * 
			 * Fleet f; if(Fleet.findByName(fleetName) == null) { f = new
			 * Fleet("", 0); f.save(); } else { f = Fleet.findByName(fleetName);
			 * f.save(); }
			 * 
			 * Type t; if(Type.findByName(typeName) == null) { t = new
			 * Type(typeName, description); t.save(); } else { t =
			 * Type.findByName(typeName); t.description = description; t.save();
			 * }
			 * 
			 * Owner o; if(Owner.findByName(ownerName) == null) { o = new
			 * Owner(ownerName, ownerEmail); o.save(); } else { o =
			 * Owner.findByName(ownerName); o.save(); } Vehicles v.typev = t;
			 * 
			 * v.owner = o;
			 * 
			 * v.fleet = f;
	...
	public Result listTruckCompositions() {
		// List<TruckComposition> allTruckComps=new
		// ArrayList<TruckComposition>();
	...
	public Result removeTrailer(long vId) {
		...
		if (trailerToRemove == null) {
			...
			System.out
					.println("TRAILER IS NULL AT REMOVING TRAILER ////////////////");
		}
		try {
	...
	public Result moveUp(long vId, long tId) {
		...
		try {
			...
			for (Vehicle vhc : tc.truckVehicles) {
				System.out.println("VEHICLE INDEXES BEFORE REORDER " + vhc.vid
						+ "(" + vhc.truckComposition.truckVehicles.indexOf(vhc)
						+ ")");
			}
			...
			System.out
					.println("ISPISUJEM NIZ VOZILA POSLE ZAMJENE MJESTA://////////");
			for (int i = 0; i < vehiclesArray.length; i++) {
				System.out.println(vehiclesArray[i].name + "///////////");
			}
			ArrayList<Vehicle> vehiclesList = new ArrayList<Vehicle>(
					Arrays.asList(vehiclesArray));
			...
			for (Vehicle vhc : tc.truckVehicles) {
				...
				System.out.println("VEHICLE INDEXES AFTER REORDER " + vhc.vid
						+ "(" + vhc.truckComposition.truckVehicles.indexOf(vhc)
						+ ")");
			}
