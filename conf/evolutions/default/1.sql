...
create table client (
  ...
  email                     varchar(255),
  client_id                 bigint,
  constraint uq_client_client_id unique (client_id),
...
alter table client add constraint fk_client_client_1 foreign key (client_id) references client (id) on delete restrict on update restrict;
create index ix_client_client_1 on client (client_id);
...
alter table type add constraint fk_type_description_9 foreign key (description_id) references description (id) on delete restrict on update restrict;
create index ix_type_description_9 on type (description_id);
